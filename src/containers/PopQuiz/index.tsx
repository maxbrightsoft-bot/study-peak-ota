...
import { View, Text, TouchableOpacity, ScrollView, TextInput, Modal, ActivityIndicator, Alert, Share } from 'react-native';
                          ...
                          <TouchableOpacity 
                            ...
                            onPress={async () => {
                              try {
                                ...
                                const shareUrl = `${baseUrl}/pop-quiz/${quiz.code}`;
                                await Share.share({ message: shareUrl });
                              } catch (error) {
                                console.error('Failed to share pop quiz link', error);
                              }
                            }}
                          >
                            <Ionicons name="share-social-outline" size={20} color={palette.main[500]} />
                          </TouchableOpacity>
                        )}
                        <TouchableOpacity
                ...
                <>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      try {
                        const baseUrl = (STUDENT_URL || 'https://student.studypeak.io').replace(/\/$/, '');
                        const shareUrl = `${baseUrl}/pop-quiz/${selectedQuiz.code}`;
                        await Share.share({
                          message: shareUrl,
                        });
                      } catch (error) {
                  ...
                  >
                    ...
                    <Text style={styles.sheetActionText}>{t('share_link') || '공유하기'}</Text>
                    <Ionicons name="chevron-forward" size={16} color={palette.grey[400]} />
                  </TouchableOpacity>
                </>
              )}
