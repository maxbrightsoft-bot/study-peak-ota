...
import { Question, QuestionData, TextbookResult } from "@/utils/types"
import { useTranslation } from "react-i18next"
import useAuthStore from "@/store/useAuthStore"
import { getErrorMessage, toast } from "@/utils/helpers"
import { apiUploadImageFile } from "@/containers/Chat/apiClient/conversationService"
import useNotes from "@/containers/ExamResult/hooks/useNotes"
import { createNoteApi, deleteNoteApi, updateNoteApi } from "@/containers/ExamResultList/apiClients/noteService"
import { ExamNoteDialogProps } from "@/containers/IncorrectAnswerNotes/configs/interfaces"
import { pick } from "@react-native-documents/picker"

interface UseTextbookResultNoteParams {
    questionOptions: any[]
...
const useTextbookResultNote = ({
    questionOptions,
    handleSelectQuestion,
    studentTextbookSessionId,
    textbookResult
}: UseTextbookResultNoteParams) => {
    ...
    useEffect(() => {
        if (!user?.id || !studentTextbookSessionId) {
    ...
    }, [user?.id, studentTextbookSessionId])
...
export default useTextbookResultNote
