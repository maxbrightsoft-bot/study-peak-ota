...
const TextbookAnswerItem: FC<TextbookAnswerItemProps> = ({ data, nextData, isFirst, isLast, effectSize }) => {
  ...
  const getOverallRateText = () => {
    ...
    return (
      <View style={styles.overallContainer}>
        <Text style={[styles.overallRate, { color: overallColor }]}>{`${data.overallCorrectRate?.toFixed(2)}%`}</Text>
  ...
  const borderBottomColor =
    data?.questionGroupIndex !== nextData?.questionGroupIndex && !isFirst && !isLast ? '#E4E7EC' : 'transparent'
      ...
      <View style={styles.row}>
        <View style={styles.column1}>
          <View style={styles.questionInfo}>
            {getStarIcon()}
            <Text style={styles.questionOrder}>{t('number_question', { number: questionNumber })}</Text>
          </View>
        </View>

        <View style={styles.column2}>
          <View style={styles.statusContainer}>{getAnswerStatusIcon()}</View>
        </View>

        <View style={styles.column3}>
          <View style={styles.timeContainer}>{getDurationText()}</View>
        </View>

        <View style={styles.column4}>
          <View style={styles.timeContainer}>{getComparisonText()}</View>
        </View>

        <View style={styles.column5}>
          <View style={styles.rateContainer}>{getOverallRateText()}</View>
